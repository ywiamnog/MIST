...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.filters = [
            feature_size,               # First encoder block
            2 * feature_size,           # Second encoder block
            4 * feature_size,           # Third encoder block
            8 * feature_size,           # Fourth encoder block
            16 * feature_size,          # Fifth encoder block
        ]
        ...
        if self.deep_supervision:
            ...
            self.deep_supervision_heads = nn.ModuleList([UnetOutBlock(spatial_dims, self.filters[i], self.out_channels, dropout=self.dropout_rate) for i in range(self.deep_supr_num)])
    ...
    def forward(self, x):
        ...
        if self.deep_supervision and self.training:
            ...
            for i in range(len(self.deep_supervision_heads)):
                feature_map = self.deep_supervision_heads[i](heads[i])
                output_deep_supervision.append(interpolate(feature_map, x.shape[2:]))
        if self.training:
            output = dict()
            output["prediction"] = self.out(out)
